...
- **Sub-100ms cold-start for namespaced commands.** v0.1.0 closed with the structural split landed: cli.ts dispatches via dynamic imports, tsup `splitting: true` emits one chunk per namespace, and `ascend --version` is now ~40ms (was 620ms — 15x). But namespaced invocations like `ascend todo list` still pay ~700-900ms because the third-party deps (commander, cli-table3, date-fns, picocolors, @inquirer/prompts, open) stay external (CJS interop bug `Dynamic require of "events" is not supported` when bundling them into ESM). Real fixes: (1) switch tsup `format: ["cjs"]` AND replace `import.meta.url` with `__dirname` in cli.ts so the CJS Node resolver fast-path is used, OR (2) ship a per-command-namespace separate npm package model so each chunk has its own minimal dep tree, OR (3) move to a faster runtime (Bun, Deno).
